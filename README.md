...
- Create and search a local project gallery, filter favorites, and move rough
  frames to a recoverable trash view.
- Organize frames into carousels with cover, content, note, and closing roles;
  switch between Editorial and AI Engineer themes; and open a reader preview
  that advances through the whole carousel.
- Import Vertica-style `slides[]` JSON or Ryu's editable carousel JSON. Export
  the current frame as PNG, JPEG, or WebP, or export the complete editable
  carousel document as JSON.
...
The app has no sidecar and does not call a provider directly. A browser-only
preview uses its local demo workspace; a Companion host starts with an empty,
node-owned gallery and persists only through the host storage bridge. The editor
clearly reports which generation or upload capability is unavailable instead of
turning a missing bridge into a successful-looking result.
The browser-only gallery displays a **Preview data** banner so sample projects
cannot be mistaken for node-owned work.

## Star History

<a href="https://github.com/amajorai/ryu-slides/stargazers">
  <picture>
