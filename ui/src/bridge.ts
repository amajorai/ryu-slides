import { carouselPrompt, normalizeState, serializeState } from "./model";
...
export async function loadState(): Promise<SlidesState> {
	const bridge = window.ryu;
	if (bridge?.storage?.get) {
		try {
			const value = await bridge.storage.get({
				key: STORAGE_KEY,
			...
			// Fall through to the local preview copy.
		}
	}
	...
	try {
		return normalizeState(JSON.parse(local));
	} catch {
		return normalizeState(null);
	}
}
